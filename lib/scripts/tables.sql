...
SELECT * FROM (
  WITH table_columns AS (
    SELECT attrelid, jsonb_object_agg(attname::text, atttypid::regtype) as types, array_agg(DISTINCT attname::text) AS columns
  ...
  SELECT t.table_schema AS schema,
    ...
    jsonb_object_agg(attname::text, atttypid::regtype) as types,
