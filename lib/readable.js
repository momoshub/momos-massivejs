...
Readable.prototype.join = function (definition) {
  ...
  const reducer = (acc, val, key) => {
    ...
    if (!on) {
      const parentReadable = val.parentReadable || this;
      const parentAlias = val.parentAlias || this.delimitedFullName;
      const candidateFks = _.chain(readable.fks)
        ...
        .reduce((allJoinPredicates, fk) => {
          ...
          if (fk.origin_schema === readable.schema && fk.origin_name === readable.name) {
  ...
  const joinRelations = definition.joinRelations;
  ...
  const clone = new Proxy(this, {
    get: (target, prop) => {
      switch (prop) {
        ...
        case 'joinSchema': return definition;
