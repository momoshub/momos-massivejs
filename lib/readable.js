 ...
 * @param {String} alias - The alias of the relation to which the criteria
 * apply.
 ...
 */
Readable.prototype.disjunct = function (criteria, alias, offset) {
  return _.reduce(criteria, (disjunction, subconditions) => {
    const conjunction = this.conjunct(subconditions, alias, disjunction.offset + disjunction.params.length);
 ...
 * queries after the ON.
 */
Readable.prototype.conjunct = function (criteria, alias, offset = 0) {
  return _.reduce(criteria, (conjunction, val, joinKey) => {
    if (joinKey === 'or') {
      const disjunction = this.disjunct(val, alias, offset);
    ...
    let parsedJoinKey = parseKey.withAppendix(
      `${alias}.${joinKey}`,
      this,
      ops,
      val,
    ...
    if (_.isString(val)) {
      // val can be a column elsewhere in the join corresponding to joinKey
      const sourceKey = parseKey(val, this);

      if (this.columns.some(c => c.fullName === sourceKey.path && !sourceKey.remainder)) {
        conjunction.predicates.push(`${parsedJoinKey.lhs} = ${sourceKey.path}`);

        return conjunction;
      }
    }

    // val is not a column, therefore a constant
    if (parsedJoinKey.appended.mutator) {
      parsedJoinKey = parsedJoinKey.appended.mutator(parsedJoinKey);
    }

    conjunction.params = conjunction.params.concat(parsedJoinKey.params);
    conjunction.predicates.push(`${parsedJoinKey.lhs} ${parsedJoinKey.appended.operator} ${parsedJoinKey.value}`);
...
Readable.prototype.solveEtCoagula = function (criteria, alias, offset = 0) {
  const assemblage = this.conjunct(criteria, alias, offset);

  assemblage.predicates = `(${assemblage.predicates.join(' AND ')})`;
...
Readable.prototype.join = function (definition) {
  ...
  joinRelations = joinRelations.map(joinRelation => {
    ...
    joinRelation.on = clone.solveEtCoagula(joinRelation.on, joinRelation.alias, offset);
