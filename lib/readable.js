...
Readable.prototype.conjoin = function (criteria, offset, kind, ...args) {
  return _.reduce(criteria, (conjunction, val, key) => {
    ...
    if (_.isString(val)) {
      // val can be another Readable column name to compare against
    ...
    if (kind === this.forDoc) {
      condition = where.docGenerator(condition, key);
    } else {
      // join or table where
 ...
 *
 * @param {Object} criteria - A map of column names to values.
 * @param {Number} offset - Where to start counting $n placeholders.
 * @param {Symbol} kind - forJoin, forWhere, or forDoc.
