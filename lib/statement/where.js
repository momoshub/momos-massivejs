...
const tableGenerator = function (condition) {
  ...
  if (condition.isJSON &&  _.isArray(condition.value)) {
    condition.value = condition.value.map(v => v.toString());
  } else if (condition.isJSON && condition.value) {
    ...
    */
    ...
    if (_.isBoolean(condition.value)) {
      ...
      if (_.isArray(condition.value)) {
        condition.params.push(condition.value.map(v => v.toString()));
      } else {
        condition.params.push(condition.value.toString());
      }
 ...
 */
const generateDisjunction = (conditions, source, offset, generator) => {
  return _.reduce(conditions, (disjunction, subconditions) => {
    ...
    const conjunction = generateConjunction(subconditions, source, disjunction.offset + disjunction.params.length, generator);
 ...
 */
const generateInnerConjunction = (conditions, source, offset, generator) => {
  return _.reduce(conditions, (conjunction, subconditions) => {
    ...
    /* eslint-disable no-use-before-define */
    const innerConjunction = generateConjunction(subconditions, source, conjunction.offset + conjunction.params.length, generator);
    /* eslint-enable no-use-before-define */
 ...
 */
const generateConjunction = (criteria, source, offset, generator) => {
  return _.reduce(criteria, (conjunction, value, key) => {
    if (key === 'or') {
      const disjunction = generateDisjunction(value, source, conjunction.offset + conjunction.params.length, generator);
      ...
      const inner_conjunction = generateInnerConjunction(value, source, conjunction.offset + conjunction.params.length, generator);
    ...
    const condition = parseKey.withAppendix(
      generator === 'docGenerator' ? `body.${key}` : key,
      source,
      ops,
      value,
 ...
 * predicate mapping.
 */
exports = module.exports = function where (source, criteria, offset = 0, generator = 'tableGenerator') {
  ...
  if (Object.prototype.hasOwnProperty.call(criteria, 'conditions') && Object.prototype.hasOwnProperty.call(criteria, 'params')) {
    ...
    if (_.isPlainObject(criteria.where) && !_.isEmpty(criteria.where)) {
      const subWhere = where(source, criteria.where || {}, criteria.params.length, criteria.nestedGenerator || generator);
  ...
  const conjunction = generateConjunction(criteria, source, offset, generator);
