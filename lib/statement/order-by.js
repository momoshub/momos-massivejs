 ...
 * @param {Entity} source - The database object for which a query is being generated.
 ...
 */
exports = module.exports = (order, source, useBody) => {
  // TODO inline in Select
  if (!order) { return ''; }
  ...
  return 'ORDER BY ' + _.reduce(order, function (acc, val) {
    ...
    acc.push(exports.fullAttribute(val, source, useBody) + direction + nulls);

    return acc;
  }, []).join(',');
};

exports.fullAttribute = function (orderObj, source, useBody) {
  let field;
  ...
  if (orderObj.expr) {
    ...
    const parsed = parseKey(orderObj.field, source, jsonAsText);

    field = parsed.lhs;
  }

  // TODO just use casts in parseKey?
  if (orderObj.type) {
