...
// module-level mutex and introspector storage to ensure they're only read from
// disk one time
let moduleMutex;
const introspectors = {};
 ...
 *
 * @return {Promise} A promise resolving into a name:QueryFile map.
 */
Database.prototype.getIntrospectors = function () {
  if (!_.isEmpty(introspectors)) {
    return this.$p.resolve(introspectors);
  }

  return this.$p((resolve, reject) => {
    ...
    files.forEach(file => {
      const introspector = new pgp.QueryFile(file, {minify: true});

      if (introspector.error) {
        throw introspector.error;
      }

      introspectors[path.basename(file)] = introspector;
    });
...
Database.prototype.reload = function () {
  ...
  const initPromises = [
    ...
    // ensure multiple connection attempts in a Promise.all() still only load
    // the introspection scripts from disk once
    moduleMutex.acquire()
      .then(this.getIntrospectors.bind(this))
      .then(scripts => this.loader.queryFiles = scripts)
      .then(moduleMutex.release.bind(moduleMutex))
  ];
 ...
 * Execute a query.
 *
 * @param {Statement|String} query - One of the four statement objects, or a
 * string containing a prepared SQL statement.
